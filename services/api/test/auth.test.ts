...
describe("device ownership", () => {
  ...
  it("issues a signed owner token, rejects tampering and expires it", async () => {
    ...
    await expect(
      ownerHashFrom(request, secret, issuedAt.getTime()),
    ).resolves.toMatch(/^[a-f0-9]{64}$/);
    const replacement = credential.token.endsWith("A") ? "B" : "A";
    const tampered = new Request("https://studio.test/v1/artifacts", {
      headers: {
        "x-device-token": `${credential.token.slice(0, -1)}${replacement}`,
      },
    });
    await expect(
      ownerHashFrom(tampered, secret, issuedAt.getTime()),
  ...
  it("refreshes a recently expired signed token without changing its owner", async () => {
    ...
    const originalRequest = new Request(
      "https://studio.test/v1/devices/refresh",
      {
    ...
    const refreshedRequest = new Request("https://studio.test/v1/artifacts", {
      headers: { "x-device-token": refreshed.token },
    });

    expect(refreshed.expiresAt).toBe("2028-08-21T00:00:00.000Z");
    await expect(
      ownerHashFrom(refreshedRequest, secret, refreshAt.getTime()),
    ).resolves.toBe(
  ...
  it("exposes the verified owner expiry while rejecting expired normal operations", async () => {
    ...
    await expect(
      ownerCredentialFrom(request, secret, issuedAt.getTime()),
