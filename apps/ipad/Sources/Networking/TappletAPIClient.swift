...
protocol TappletHTTPTransport: Sendable {
    func data(for request: URLRequest) async throws -> (Data, HTTPURLResponse)
}

struct URLSessionTappletTransport: TappletHTTPTransport, @unchecked Sendable {
    let session: URLSession
    init(session: URLSession = .shared) { self.session = session }
    func data(for request: URLRequest) async throws -> (Data, HTTPURLResponse) {
        let (data, response) = try await session.data(for: request)
        guard let response = response as? HTTPURLResponse else { throw TappletAPIError.invalidResponse }
...
actor KeychainDeviceTokenStore: DeviceTokenProviding {
    ...
    private let service = "sg.tinkertanker.Tapplet"
    ...
    func token() throws -> String {
        ...
        else { throw TappletAPIError.registrationRequired }
        cached = value
        return value
    }
    ...
    func storeToken(_ value: String) throws {
        ...
        if status == errSecItemNotFound {
            ...
            guard SecItemAdd(item as CFDictionary, nil) == errSecSuccess else { throw TappletAPIError.registrationRequired }
...
struct TappletAPIClient: TappletAPI, Sendable {
    let baseURL: URL
    ...
    func hasDeviceCredential() async -> Bool {
        ...
        catch let error as TappletAPIError { return !error.requiresRegistration }
        catch { return true }
    }
    ...
    func source(revision: ArtifactRevision) async throws -> ArtifactSource {
        ...
        guard let html = String(data: data, encoding: .utf8) else { throw TappletAPIError.invalidResponse }
    ...
    func downloadAsset(id: String) async throws -> DownloadedAppletAsset {
        ...
        guard let mediaType, mediaType.hasPrefix("image/"), !data.isEmpty else {
            throw TappletAPIError.invalidResponse
        }
        return DownloadedAppletAsset(data: data, mediaType: mediaType)
    }
    ...
    private func request(_ route: String, method: String) throws -> URLRequest {
        guard let url = URL(string: route, relativeTo: baseURL)?.absoluteURL else { throw TappletAPIError.invalidURL }
    ...
    private func execute(_ request: URLRequest, mayRefresh: Bool = true) async throws -> (Data, HTTPURLResponse) {
        ...
        catch { throw TappletAPIError.transport(error.localizedDescription) }
        do { try validate(response, payload: data) }
        catch let error as TappletAPIError where mayRefresh && error.requiresRefresh {
    ...
    private func decode<T: Decodable>(_ data: Data) throws -> T {
        ...
        catch { throw TappletAPIError.decoding(error.localizedDescription) }
    }
